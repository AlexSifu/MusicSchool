...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const res = await apiFetch(API_BASE_URL+`/students/${id}`);
    if (!res || !res.ok) {
    ...
    birthdateInput.value = formatearFechaISO(student.birthDate);
    ...
    document.getElementById("editForm").addEventListener("submit", async (e) => {
        ...
        const updateRes = await apiFetch(API_BASE_URL+`/students/${id}`, {
            method: "PUT",
...
async function loadSchools() {
    try {
        const res = await apiFetch(API_BASE_URL + "/students/Schools", {
            method: "GET",
        });
